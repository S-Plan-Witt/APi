...
class User {
     ...
     * @param permissions
     */
    constructor(firstName = "", lastName = "", username = "", id = 0, type = "", courses, active = null, mails = null, devices = null, secondFactor = 0, permissions = new Permissions()) {
        this.displayName = "";
        this.active = active;
        this.id = id;
     ...
     */
    createUserFromLdap(username) {
        return new Promise(async function (resolve, reject) {
            let user = await ldap_1.Ldap.getUserByUsername(username);
            let conn = await pool.getConnection();
            try {
    ...
    generateToken() {
        ...
        return new Promise(async function (resolve, reject) {
            ...
            try {
                resolve(await jwt_1.Jwt.createJWT(username, type, tokenId));
            }
            catch (e) {
                //TODO add logger
                reject(e);
    ...
    verifyPassword(password) {
        ...
        return new Promise(async function (resolve, reject) {
            try {
                await ldap_1.Ldap.checkPassword(username, password);
                resolve("Ldap");
            }
            catch (err) {
                //TODO add logger
     ...
     */
    static getCourses(userId, userType, username) {
        return new Promise(async function (resolve, reject) {
            let conn = await pool.getConnection();
            try {
                let courses = [];
                if (userType == "student") {
                    ...
                    await conn.end();
                    rows.forEach((row) => {
                        courses.push(new timeTable_1.Course(row.grade, row.subject, row.group, row.exams));
                    });
                }
    ...
    static getEMails(userId) {
        return new Promise(async function (resolve, reject) {
            ...
            try {
                ...
                await conn.end();
                rows.forEach((row) => {
                    mails.push(new eMail_1.EMail(userId, row["mail"], row["confirmed"], row["added"]));
                });
                resolve(mails);
            }
            catch (err) {
     ...
     */
    static getStudentDevicesByCourse(course) {
        return new Promise(async function (resolve, reject) {
            let conn = await pool.getConnection();
            try {
                let rows = await conn.query("SELECT splan.student_courses.*, splan.devices.* FROM splan.student_courses " +
                    ...
                    "WHERE (`grade`=? && `subject`=? && `group`=?)", [course.grade, course.subject, course.group]);
                let devices = [];
                rows.forEach((row) => {
    ...
    static find(filter) {
        return new Promise(async function (resolve, reject) {
            try {
                let list = await ldap_1.Ldap.searchUser(filter);
                resolve(list);
            }
            catch (e) {
                logger.log({
     ...
     */
    static getUserByCalToken(token) {
        return new Promise(async function (resolve, reject) {
            let conn = await pool.getConnection();
            try {
    ...
    static getAllStudentsLDAP() {
        return new Promise(async function (resolve, reject) {
            try {
                let users = await ldap_1.Ldap.getAllStudents();
                resolve(users);
            }
            catch (e) {
                console.log(e);
...
class UserFilter {
    constructor(username, firstName, lastName, birthday) {
        this.username = username;
        this.firstName = firstName;
        this.lastName = lastName;
